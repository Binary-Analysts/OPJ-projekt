...
def build_lstm(vocab_size, emb_matrix, num_classes):
    model = Sequential([
...
def run_experiment(train_df, test_df, model_name):
    ...
    model = build_lstm(len(tokenizer.word_index) + 1, emb_matrix, len(le.classes_))
...
for i, test in enumerate([
    ...
    acc, p, r, f1 = run_experiment(
        ...
        model_name=f"LSTM_ALL_T{i+1}"
    )

    results.append(["LSTM", f"ALL-T{i+1}", acc, p, r, f1])
