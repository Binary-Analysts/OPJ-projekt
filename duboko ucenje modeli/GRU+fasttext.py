...
# 2. 80% Train -> 20% Val, 20% Test
sgkf = StratifiedGroupKFold(n_splits=5, shuffle=True, random_state=seed)
train_idx, test_idx = next(sgkf.split(df,y=df["label"], groups=df["review_id"]))
...
final_train_idx, val_idx = next(sgkf2.split(df_train_tmp,y=df_train_tmp["label"],groups=df_train_tmp["review_id"]))
...
for epoch in range(n_epochs):
    ...
    if valid_loss < best_valid_loss:
        ...
        torch.save(model.state_dict(), "croatian_gru.pt")
        
        with open("vokabular_gru.pkl", "wb") as f:
            pickle.dump({
...
model.load_state_dict(torch.load("croatian_gru.pt"))
